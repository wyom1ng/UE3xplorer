...
public struct FName
{
    ...
    private static Dictionary<string, int> nameMap = new(StringComparer.OrdinalIgnoreCase);
    ...
    public void ResolveFrom(string[] names)
    {
        if (nameMap.Count == 0)
        {
            foreach (EName name in Enum.GetValues(typeof(EName)))
            {
                nameMap[name.ToString()] = (int)name;
            }
